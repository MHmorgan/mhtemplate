...
impl Evaluator<String> for Expr {
    fn evaluate(&self, ctx: &Context) -> Result<String> {
        match self {
            ...
            Expr::Unknown => {
                Err(TmplError::new("cannot evaluate unknown expression"))
            },
        }
    }
}
    ...
    fn parse_expression(&mut self) -> Result<Expr> {
        ...
        'parse: while let Some(tok) = self.next_token() {
            match tok {
                ...
                // increment
                ExprToken::Increase => {
                    expr = Expr::Inc { expr: Box::new(expr) };
                },
                ...
                // post decrement
                ExprToken::Decrease => {
                    expr = Expr::Dec { expr: Box::new(expr) };
                },
...
mod tests {
    ...
    fn test_expression() {
        ...
        let cnst    = Expr::Const    { val: String::from("4") };
        let var     = Expr::Variable { ident: String::from("myvar") };
        let neg     = Expr::Neg      { expr: Box::new(cnst.clone()) };
        let not     = Expr::Not      { expr: Box::new(var.clone()) };
        let preinc  = Expr::Inc      { expr: Box::new(var.clone()) };
        let postinc = Expr::Inc      { expr: Box::new(var.clone()) };
        let predec  = Expr::Dec      { expr: Box::new(var.clone()) };
        let postdec = Expr::Dec      { expr: Box::new(var.clone()) };
        let add     = Expr::Add      { lhs: Box::new(cnst.clone()), rhs: Box::new(var.clone()) };
        let mul     = Expr::Mul      { lhs: Box::new(neg.clone()), rhs: Box::new(preinc.clone()) };
        let sub     = Expr::Sub      { lhs: Box::new(add.clone()), rhs: Box::new(mul.clone()) };
        let div     = Expr::Div      { lhs: Box::new(predec.clone()), rhs: Box::new(postdec.clone()) };
        let mod_    = Expr::Add      { lhs: Box::new(postinc.clone()), rhs: Box::new(div.clone()) };
        let concat  = Expr::Concat   { lhs: Box::new(sub.clone()), rhs: Box::new(mod_.clone()) };
        let neq     = Expr::Neq      { lhs: Box::new(concat.clone()), rhs: Box::new(not.clone()) };

        // Test expression string
        let expr_str = String::from("(((4 + $myvar) - -4 * $myvar++) . ($myvar+++$myvar--/$myvar--)) != !$myvar");
