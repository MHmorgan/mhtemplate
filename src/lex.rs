...
use crate::{Result,TmplError,Expr,ExpressionFactory};
...
impl Scanner {
    ...
    fn push_text(&mut self) {
        ...
        if !txt.is_empty() {
            self.lexemes.push_back(Lexeme::Text { text: String::from(txt) });
        }
    ...
    pub fn scan_all(&mut self) -> Result<()> {
        ...
        if !self.current.is_empty() {
            ...
            self.lexemes.push_back(Lexeme::Text { text: String::from(s) });
        }
    ...
    fn handle_statement(&mut self) -> Result<()> {
        ...
        if let Some(m) = STMT_SET_RE.captures(&txt) {
            ...
            // Add lexeme
            self.lexemes.push_back(Lexeme::Set { ident: String::from(ident), val: Box::new(expr) });
            return Ok(())
        }
        ...
        if let Some(m) = STMT_FOR_RE.captures(&txt) {
            ...
            let expr  = ExpressionFactory::new::<VecDeque<char>>(expr.chars().collect()).parse()?;
            // Add lexeme
            self.lexemes.push_back(Lexeme::For { ident: String::from(ident), expr: Box::new(expr) });
            return Ok(())
        }
        ...
        if let Some(m) = STMT_FORCNT_RE.captures(&txt) {
            ...
            let expr  = ExpressionFactory::new::<VecDeque<char>>(expr.chars().collect()).parse()?;
            // Add lexeme
            self.lexemes.push_back(Lexeme::ForCnt { ident: String::from(ident), ident_i: String::from(ident_i), expr: Box::new(expr) });
            return Ok(())
        }
        ...
        if let Some(m) = STMT_REPEAT_RE.captures(&txt) {
            ...
            let expr  = ExpressionFactory::new::<VecDeque<char>>(expr.chars().collect()).parse()?;
            // Add lexeme
            self.lexemes.push_back(Lexeme::Repeat { expr: Box::new(expr) });
            return Ok(())
        }
        ...
        if let Some(m) = STMT_IF_RE.captures(&txt) {
            ...
            let expr  = ExpressionFactory::new::<VecDeque<char>>(expr.chars().collect()).parse()?;
            // Add lexeme
            self.lexemes.push_back(Lexeme::If { expr: Box::new(expr) });
            return Ok(())
        }
        ...
        if let Some(m) = STMT_ELIF_RE.captures(&txt) {
            ...
            let expr  = ExpressionFactory::new::<VecDeque<char>>(expr.chars().collect()).parse()?;
            // Add lexeme
            self.lexemes.push_back(Lexeme::Elif { expr: Box::new(expr) });
            return Ok(())
        }
    ...
    fn handle_expression(&mut self) -> Result<()> {
        ...
        self.lexemes.push_back(Lexeme::Expr { expr: Box::new(expr) });
        Ok(())
    }
    ...
    fn error<S: ToString>(&self, txt: S) -> TmplError {
        TmplError::new(format!("at line {}, col {}: {}",
                               self.line, self.col, txt.to_string()))
    }
}
